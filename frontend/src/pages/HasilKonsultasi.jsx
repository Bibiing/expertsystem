...
import { ClipboardList, CheckCircle2, AlertTriangle, Activity, Bug, Sprout, ArrowLeft } from "lucide-react";
...
function HasilDiagnosis() {
  ...
  const topDisease = diagnosisResult.length > 0 ? diagnosisResult[0] : null;
    ...
    >
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <button onClick={() => navigate("/konsultasi")} className="flex items-center gap-2 text-emerald-100 hover:text-white transition-colors duration-300 text-sm sm:text-base">
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-2 sm:gap-3">
            {selectedGejala.map((gejala, index) => (
              <div key={gejala._id} className="flex items-start gap-3 bg-slate-50 p-3 sm:p-4 rounded-lg border border-slate-100">
        ...
        {topDisease ? (
          ...
          >
            <p>Penyakit tidak dapat diidentifikasi berdasarkan gejala yang dipilih.</p>
          </motion.div>
        )}
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-start gap-2 sm:gap-3 flex-1 min-w-0">
                      <span className="shrink-0 text-lg sm:text-xl font-bold text-slate-400">#{index + 2}</span>
                      <h3 className="text-slate-800 font-semibold text-base sm:text-lg break-words">{penyakit.name}</h3>
                    </div>
                    <span className="font-bold text-base sm:text-lg ml-2 shrink-0 text-emerald-600">{(penyakit.cf * 100).toFixed(1)}%</span>
                  </div>
